...
inlineTime = c(8.83, 8.59, 8.73, 8.97, 10.7)
offlineTime = c(592, 574, 577, 550, 510)
...
p1 <- ggplot() + 
  ...
  geom_text(aes(x = x1_offset, y = round(inlineTime, 1)), hjust = 0,
            vjust = 0.4, label=inlineTime, angle = 90, size = 12, nudge_y = 1)

p2 <- ggplot() + 
  # 坐标轴显示范围
  coord_cartesian(xlim =c(-0.2, 4.5), ylim = c(380, 900)) +
  # 设置y坐标标签
  ...
  geom_text(aes(x = x2_offset, y = round(offlineTime, 1)), hjust = 0,
            vjust = 0.4, label=offlineTime, angle = 90, size = 12, nudge_y = 20.5)
