...
inlineTime = c(12.9, 12.8, 13.0, 13.1, 13.5)
offlineTime = c(27.4, 26.9, 25.5, 19.5, 18.6)
...
p1 <- ggplot() + 
  # 坐标轴显示范围
  coord_cartesian(xlim =c(-0.2, 4.5), ylim = c(1.65, 42)) +
  # 设置y坐标标签
  ...
  geom_text(aes(x = x1_offset, y = round(inlineTime, 1)), hjust = 0,
            vjust = 0.4, label=inlineTime, angle = 90, size = 12, nudge_y = 0.7) +
  geom_text(aes(x = x2_offset, y = round(offlineTime, 1)), hjust = 0,
            vjust = 0.4, label=offlineTime, angle = 90, size = 12, nudge_y = 0.7)
