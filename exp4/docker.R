...
p1 <- ggplot() + 
  # 坐标轴显示范围
  coord_cartesian(xlim =c(-0.2, 4.5), ylim = c(0.143, 3.1)) +
  # 设置y坐标标签
  ...
  geom_text(aes(x = x1_offset, y = round(inlineTime, 2)), hjust = 0,
            vjust = 0.4, label=round(inlineTime, 2), angle = 90, size = 12, nudge_y = 0.13)

p2 <- ggplot() + 
  # 坐标轴显示范围
  coord_cartesian(xlim =c(-0.2, 4.5), ylim = c(9, 40)) +
  # 设置y坐标标签
  ...
  geom_text(aes(x = x2_offset, y = round(offlineTime, 2)), hjust = 0,
            vjust = 0.4, label=round(offlineTime, 2), angle = 90, size = 12, nudge_y = 0.5)
